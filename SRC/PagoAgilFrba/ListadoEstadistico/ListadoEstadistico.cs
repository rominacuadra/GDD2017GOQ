    ...
    public partial class ListadoEstadistico : Form
    {
        ...
        private void consultarListado(int año, int mesInicio, int mesFin, string tipoListado){
            ...
            if (tipoListado == "Porcentaje de facturas cobradas por empresa")
            {
                ...
                SqlCommand cmd = new SqlCommand("select TOP 5 e.empresa_nombre,COUNT(DISTINCT fac_id)*100 / (select COUNT(DISTINCT fac_id) cantidadTotal from GOQ.Empresa e1 inner join GOQ.Factura f1 on(e1.ID_empresa = f1.fac_empresa_id) where e1.ID_empresa = f.fac_empresa_id) PorcentajeCobrado from GOQ.Factura f inner join GOQ.Pago_Factura pf on(f.fac_id = pf.pago_fac_fac_id) inner join GOQ.Pago p on(p.pago_id = pf.pago_fac_pago_id) inner join GOQ.Empresa e on(e.ID_empresa = f.fac_empresa_id) where year(p.pago_fecha_cobro) = @año and month(p.pago_fecha_cobro) between @mesInicio and @mesFin AND fac_id in (select fac_id from GOQ.Factura f join GOQ.Pago_factura pf on(f.fac_id = pf.pago_fac_fac_id) group by fac_id having COUNT(pf.pago_fac_fac_id) > (select COUNT(dev_fac_id) from GOQ.Devolucion where dev_fac_id = f.fac_id )) group by f.fac_empresa_id,e.empresa_nombre order by 2 DESC",
            ...
            else if (tipoListado == "Clientes con mas pagos")
            {
                ...
                SqlDataReader reader = null;
                SqlCommand cmd = new SqlCommand("select TOP 5 c.cli_apellido+','+c.cli_nombre ApellidoNombre, COUNT(DISTINCT pf.pago_fac_fac_id) cantidadPagos from GOQ.Cliente c inner join GOQ.Pago P ON (c.cli_id = p.pago_cliente_id) inner join GOQ.Pago_Factura pf ON (p.pago_id = pf.pago_fac_pago_id) where  year(p.pago_fecha_cobro) = @año and month(pago_fecha_cobro) between @mesInicio and @mesFin and pf.pago_fac_fac_id in (select fac_id from GOQ.Factura f join GOQ.Pago_factura pf on(f.fac_id = pf.pago_fac_fac_id) group by fac_id having COUNT(pf.pago_fac_fac_id) > (select COUNT(dev_fac_id) from GOQ.Devolucion where dev_fac_id = f.fac_id ))group by c.cli_apellido, c.cli_nombre order by 2 DESC", PagoAgilFrba.ModuloGlobal.getConexion());
                cmd.Parameters.Add("año", SqlDbType.Int).Value = año;
                cmd.Parameters.Add("mesInicio", SqlDbType.Int).Value = mesInicio;
                cmd.Parameters.Add("mesFin", SqlDbType.Int).Value = mesFin;
                reader = cmd.ExecuteReader();
                ...
                SqlDataReader reader = null;
                SqlCommand cmd = new SqlCommand("select TOP 5 c.cli_apellido+','+c.cli_nombre ApellidoNombre , COUNT(DISTINCT f.fac_id)*100/(select COUNT(f1.fac_id) cantidadTotal from GOQ.Factura f1 where f1.fac_cli_id = c.cli_id) PorcentajeFacturasPagadas from GOQ.Cliente c inner join GOQ.Factura f on(c.cli_id = f.fac_cli_id) inner join GOQ.Pago_Factura pf on(pf.pago_fac_fac_id = f.fac_id) inner join GOQ.Pago p on(p.pago_id = pf.pago_fac_pago_id) where year(p.pago_fecha_cobro) = @año and month(pago_fecha_cobro) between @mesInicio and  @mesFin and fac_id in (select fac_id from GOQ.Factura f join GOQ.Pago_factura pf on(f.fac_id = pf.pago_fac_fac_id) group by fac_id having COUNT(pf.pago_fac_fac_id) > (select COUNT(dev_fac_id) from GOQ.Devolucion where dev_fac_id = f.fac_id )) group by c.cli_apellido,c.cli_nombre,c.cli_id order by 2 DESC", PagoAgilFrba.ModuloGlobal.getConexion());
                cmd.Parameters.Add("año", SqlDbType.Int).Value = año;
                cmd.Parameters.Add("mesInicio", SqlDbType.Int).Value = mesInicio;
                cmd.Parameters.Add("mesFin", SqlDbType.Int).Value = mesFin;
                reader = cmd.ExecuteReader();
