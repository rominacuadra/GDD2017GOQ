...
create table GOQ.Funcionalidad_Rol(
	fun_rol_rol_id int  not null, /* FK  REQUIRED.Rol */
	fun_rol_fun_id int not null, /* FK  REQUIRED.Funcionalidad */
...
create table GOQ.Usuario(
	...
	usu_intentos_Log int DEFAULT 0,
);

GO
create table GOQ.Rol_Usuario(
	rol_usu_rol_id int  not null,  /* FK  REQUIRED.Rol */
	rol_usu_usu_id int  not null,  /* FK  REQUIRED.Usuario */
...
add constraint FK_fun_rol_rol_id foreign key (fun_rol_rol_id) references REQUIRED.Rol(rol_id),
	constraint FK_fun_rol_fun_id foreign key (fun_rol_fun_id) references REQUIRED.Funcionalidad(fun_id);

GO
alter table GOQ.Rol_Usuario
add constraint FK_rol_usu_rol_id foreign key (rol_usu_rol_id) references REQUIRED.Rol(rol_id),
	constraint FK_rol_usu_usu_id foreign key (rol_usu_usu_id) references REQUIRED.Usuario(usu_id);
